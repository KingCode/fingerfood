(ns fingerfood.input-test
    (:require [fingerfood.input :refer :all]
              [fingerfood.command :refer [Command end? menu? page? edit? selection? up?]]
...
(deftest make-syntax-test
    (testing "default command syntax validation"
       (let [ re *command-re* 
                ...
                syn (make-syntax re parts error-fn) ] 
            (is (valid?- syn "2"))
            (is (valid? syn "22U"))
